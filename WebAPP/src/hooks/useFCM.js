...
export const useFCM = (onNewNotification) => {
  ...
  useEffect(() => {
    ...
    const initFCM = async () => {
      // 1. Chỉ tự động lấy token nếu người dùng ĐÃ cấp quyền (granted).
      // Nếu là 'default', trình duyệt (đặc biệt Safari) yêu cầu user gesture (vd: click) để gọi Notification.requestPermission()
